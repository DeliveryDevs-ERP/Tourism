...
def get_result(filters, account_details):
	...
	project_ids = {row.get("project") for row in result if row.get("project")}
	project_name_map = {}
	...
	if project_ids:
		projects = frappe.get_all(
			...
			limit_page_length=len(project_ids),
		)
		project_name_map = {p["name"]: p["project_name"] for p in projects}

	for row in result:
		proj = row.get("project")
		row["project_name"] = project_name_map.get(proj) if proj else None
	return result
